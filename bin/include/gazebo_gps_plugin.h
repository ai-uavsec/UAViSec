...
protected:
  ...
  void OnOffsetUpdate(const boost::shared_ptr<const msgs::Vector3d> &);
  void OnRotationOffsetUpdate(const boost::shared_ptr<const msgs::Vector3d> &);
...
private:
  ...
  transport::SubscriberPtr gps_offset_list_;
  transport::SubscriberPtr gps_rot_offset_list_;
  transport::PublisherPtr gps_data_pub_;
  ...
  // gps spoof parameters
  bool attack_enabled = false;
  bool rot_attack_enabled = false;

  double latitude_deg_offset_ = 0.0;
  double longitude_deg_offset_ = 0.0;
  double altitude_offset_ = 0.0;

  double degree_offset_ = 0.0;
  double degree_offset_target_ = 0.0;
  double rotation_ = 0.0;
  bool direction_needed_ = false;
  double unit_vector_east_ = 0.0;
  double unit_vector_north_ = 0.0;
  double cos_rot_ = 0.0;
  double sin_rot_ = 0.0;

  double latitude_deg_offset_target_ = 0.0;
  double longitude_deg_offset_target_ = 0.0;
  double altitude_offset_target_ = 0.0;

  std::mutex mutex_;
  std::condition_variable cv_;

  double offset_increment = 0.0000001; // 0.00001 * 5Hz/500Hz
