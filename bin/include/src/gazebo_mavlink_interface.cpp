...
bool motorattack=false;
bool motorstop=false;
bool motor_half=false;
bool attack_mag=false;
typedef const boost::shared_ptr<const msgs::Vector3d> Message;
int motor_id;
float motor_attack_speed;
// transport
transport::SubscriberPtr motorlistener;
...
void OnMotorSignal(Message &_msg)
{
  // bool BaroAttack=false;
  double signal_ = _msg->x();
  if (signal_==1.0)
    motorattack=!motorattack;
  else if (signal_==2.0)
    motor_half=!motor_half;
  else if (signal_==0.0)
    motorstop= !motorstop;
  motor_id = _msg->y();
  motor_attack_speed=_msg->z();
}
...
void GazeboMavlinkInterface::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  motorlistener = node_handle_->Subscribe("~/attack/motor",OnMotorSignal,this);
...
void GazeboMavlinkInterface::OnUpdate(const common::UpdateInfo&  /*_info*/) {
  ...
  if (received_first_actuator_) {
    ...
    if (motorattack)
      input_reference_[motor_id]=motor_attack_speed;
    for (int i = 0; i < input_reference_.size(); i++) {
      ...
      {
        turning_velocities_msg.add_motor_speed(motor_attack_speed);
      }
      else {
...
void GazeboMavlinkInterface::MagnetometerCallback(MagnetometerPtr& mag_msg) {
  ...
  if (attack_mag)
    mag_data.mag_b = Eigen::Vector3d(mag_msg->magnetic_field().y(),
    mag_msg->magnetic_field().x(), mag_msg->magnetic_field().z());
  else
