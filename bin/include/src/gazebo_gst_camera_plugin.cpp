...
bool multicast;
typedef const boost::shared_ptr<const msgs::Int> Message;

transport::SubscriberPtr streamlistener;
bool is_flip=false;
bool Is_mask=false;
void OnStreamSignal(Message &_msg)
{
  // bool BaroAttack=false;
  int signal_ = _msg->data();
  if(signal_== 1 ){
    is_flip=!is_flip;
  }
  if(signal_== 2){
...
void GstCameraPlugin::startGstThread() {
  ...
  if (useRtmp) {
    ...
    sink  = gst_element_factory_make("udpsink", nullptr);
      if(multicast){
      g_object_set(G_OBJECT(sink), "host", this->udpHost.c_str(), "port", "239.255.12.42", "auto-multicast",true,nullptr);
      }else {
...
void GstCameraPlugin::Load(sensors::SensorPtr sensor, sdf::ElementPtr sdf)
{
 ...
 if (!isfinite(this->rate)) {
  ...
  if(sdf->HasElement("multicast")){
    multicast = sdf->GetElement("multicast")->Get<bool>();
  }else {
    multicast = false;
  }
  ...
  streamlistener = node_handle_->Subscribe("~/attack/stream",OnStreamSignal,this);
                              ...
                              const std::string &format)
{
  ...
  // try to set a mask to influence the stream frame
  if(Is_mask){
    Mat mask =Mat::zeros(height, width, CV_8UC1);
    Point center(frame.cols/2, frame.rows/2);
    Point topLeft(center.x-20, center.y-20);
    Rect square(topLeft.x, topLeft.y,40,40);
    rectangle(mask,square,Scalar(255),FILLED);
