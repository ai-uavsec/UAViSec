...
void OnImuSIgnal(Message &_msg)
{
  ...
  if(signal_==1.0){
    ...
    {
    temp_increment[n]=ins;
      }
  else if (signal_==4)
    memset(temp_increment,0,sizeof(temp_increment));
  else if (signal_==5)
    imuAttack=false;
...
void GazeboImuPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  imu_data_pub_ = node_handle_->Advertise<sensor_msgs::msgs::Imu>("~/log/imu", 10);
...
void GazeboImuPlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  // if(imuAttack || one_attack)
    if (imuAttack)
    {
      // attack_increment=temp_increment;

      for (int j=0;j<6;++j)
      {
          attack_increment[j]=temp_increment[j];
      }
    }
    ...
    angular_velocity->set_x(angular_velocity_I[0]+ attack_increment[3]);
    angular_velocity->set_y(angular_velocity_I[1]+ attack_increment[4]);
    angular_velocity->set_z(angular_velocity_I[2]+ attack_increment[5]);
