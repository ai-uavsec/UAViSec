...
void SonarPlugin::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  sonar_listener = node_handle_->Subscribe("~/attack/sonar",OnSonarSignal,this);
...
void SonarPlugin::OnNewScans()
{
...
#endif
  double current_distance = parentSensor_->Range();
  ...
  if(SonarAttack)
    current_distance=current_distance + update_distance;
  if (current_distance < parentSensor_->RangeMin()) {
    current_distance = parentSensor_->RangeMin();
  } else if (current_distance > parentSensor_->RangeMax() || std::isinf(current_distance)) {
    current_distance = parentSensor_->RangeMax();
  }
  sonar_message_.set_current_distance(current_distance);
