...
class GazeboWindPlugin : public WorldPlugin {
 ...
 protected:
  ...
  void OnWindUpdate(const boost::shared_ptr<const msgs::Vector3d> &);

  void OnWindAttackUpdate(const boost::shared_ptr<const msgs::Int> &);
 ...
 private:
  ...
  transport::SubscriberPtr wind_offset_list_;
  transport::SubscriberPtr attack_enable_ptr_;
  bool attack_enabled_;
  double attack_offset_x_;
  double attack_offset_y_;
  double attack_offset_z_;
